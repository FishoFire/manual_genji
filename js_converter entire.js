...
function Converter(){
    ...
    defaultdata()
    // decompileAllRules(document.getElementById("converterdata").value, "en-US");
    lang_convert = document.getElementById("lang_convert").value 
    pasta =  decompileAllRules(document.getElementById("converterdata").value , lang_convert);
    
    
    // regex = \bA = \[
    // the \b detects empty lines the \ dealw with the bracklet
    ...
    */
    ...
    //pasta = decompileAllRules(pasta, lang_convert);
...
function ReadVectorArray(searchvar){
    var temppasta = pasta
    var entries = []
    if ( temppasta.includes("\n    " + searchvar + " = [") == false){
...
first first rule
delete all before it

while rules
    make sub var with rule:
        start till start of enxt rule
        deakl with comented out rules 
        not #   rule "x":
    
    if not @disable or @eventplayer == hostplayer {
...
compare waits and orders of

array of highest waits if any


deal with variables not found
