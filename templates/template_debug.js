...
rule("Setup and Variables")
{
	...
	actions
	{
		...
		Global.B = Count Of(Global.A) - 1;
		"KILLBALL()\r\n wait(2)\r\n pinball()"
		Global.PortalOn = Workshop Setting Toggle(Custom String("map settings"), Custom String("enable portals (control maps)"), ${portalon}, 0);
		...
		If(!Host Player.EditorOn);
			Global.Difficultyhud = Workshop Setting Combo(Custom String("map settings"), Custom String("difficulty (display hud)"), ${difficultyhud}, Array(
...
rule("Ban | Wallclimb for specific CPs <---- EDIT ME")
{
	...
	conditions
	{
		Workshop Setting Toggle(Custom String("Ban Switch"), Custom String("Ban Wallclimb for specific CPs"), ${ban_wallclimbEnabled}, 2) == True;
		Event Player.C == 0;
		Event Player.A < Count Of(Global.A) - 1;
...
rule("Ban | Bhop for specific CPs      <---- EDIT ME")
{
	...
	conditions
	{
		Workshop Setting Toggle(Custom String("Ban Switch"), Custom String("Ban Bhop for specific CPs"), ${ban_bhopEnabled}, 3) == True;
		Event Player.C == 0;
		Event Player.A < Count Of(Global.A) - 1;
		"Change \"-1\" to certain Checkpoints' number"
		${ban_bhopsCp}
...
rule("Ban | Triple Jump")
{
	...
	conditions
	{
		Workshop Setting Toggle(Custom String("Ban Switch"), Custom String("Ban Triple Jump"), ${ban_triple}, 0) == True;
		Event Player.C == 0;
		Event Player.A < Count Of(Global.A) - 1;
...
rule("Ban | Multiclimb")
{
	...
	conditions
	{
		Workshop Setting Toggle(Custom String("Ban Switch"), Custom String("Ban Multiclimb"), ${ban_multi}, 1) == True;
		Event Player.C == 0;
		Event Player.A < Count Of(Global.A) - 1;
		Event Player.climbNum > 0;
	}
...
rule("Ban | Emote Savehop")
{
	...
	conditions
	{
		Workshop Setting Toggle(Custom String("Ban Switch"), Custom String("Ban Emote Savehop"), ${ban_emote}, 3) == True;
		Event Player.C == 0;
		Event Player.A < Count Of(Global.A) - 1;
		Is Communicating Any Emote(Event Player) == True;
	}
...
rule("Ban | Create Bhop")
{
	...
	{
		Wait(1, Ignore Condition);
		Global.kaxiaotiao = Workshop Setting Toggle(Custom String("Ban Switch"), Custom String("Ban Create Bhop"), ${ban_create}, 2);
	}
}
...
rule("Ban | Deathbhop")
{
	...
	{
		Wait(1, Ignore Condition);
		Global.deathjump = Workshop Setting Toggle(Custom String("Ban Switch"), Custom String("Ban Deathbhop"), ${ban_dbhop}, 4);
	}
}
...
rule("Ban | Dash Start")
{
	...
	{
		Wait(1, Ignore Condition);
		Global.DashExploitToggle = Workshop Setting Toggle(Custom String("Ban Switch"), Custom String("Ban Dash Start"), ${ban_dashstart}, 4);
	}
}
...
disabled rule("In world text for certain Checkpoints <---- EDIT ME")
{
	...
	actions
	{
		Create In-World Text(Filtered Array(All Players(All Teams), Current Array Element.A == 0), Custom String("Text for checkpoint 0"),
			Vector(0, 0, 0), 1.200, Clip Against Surfaces, Visible To Position and String, Color(Orange), Default Visibility);
		Create In-World Text(Filtered Array(All Players(All Teams), Current Array Element.A == 1), Custom String("Text for checkpoint 1"),
			Vector(0, 0, 0), 1.200, Clip Against Surfaces, Visible To Position and String, Color(Orange), Default Visibility);
		Create In-World Text(Filtered Array(All Players(All Teams), Current Array Element.A == 2), Custom String("Text for checkpoint 2"),
			Vector(0, 0, 0), 3, Clip Against Surfaces, Visible To Position and String, Color(Orange), Default Visibility);
	}
}
...
rule("Ms. Destructo | Destroys Breakable Objects On All Maps")
{
	...
	actions
	{
		...
		Create Dummy Bot(Hero(D.Va), Team Of(First Of(Filtered Array(All Players(All Teams), Has Spawned(Current Array Element)
			&& !Is Dummy Bot(Current Array Element)))), -1, Vector(0, 200, 0), Vector(0, 0, 0));
		...
		Global.MsDestructo.MapVectorArray = Array(Array(0, Map(Hanamura), Map(Hanamura), Map(Horizon Lunar Colony), Map(Paris), Map(
			Dorado), Map(Havana), Map(Junkertown), Map(Rialto), Map(Route 66), Map(Watchpoint: Gibraltar), Map(Blizzard World), Map(
			Blizzard World), Map(Eichenwalde), Map(Eichenwalde), Map(Hollywood), Map(King's Row), Map(King's Row), Map(Ayutthaya), Map(
			Busan), Map(Busan), Map(Ilios), Map(Ilios), Map(Lijiang Control Center), Map(Lijiang Control Center), Map(Lijiang Garden), Map(
			Lijiang Garden), Map(Lijiang Night Market), Map(Lijiang Night Market), Map(Nepal), Map(Nepal), Map(Nepal), Map(Oasis), Map(
			Oasis), Map(Oasis), Map(Black Forest), Map(Black Forest), Map(Castillo), Map(Castillo), Map(Castillo), Map(
			Kanezaka), Map(Petra), Map(Volskaya Industries), Map(Practice Range), Map(Ilios), Map(Busan), Map(Lijiang Tower), Map(
			Lijiang Tower), Map(Nepal), Map(Oasis), Map(Malevento)), Array(Vector(15.759, 30.576, -27.201)), Array(Vector(15.759, 30.576,
			-27.201)), Array(Vector(31.759, 40.003, -59.476)), Array(Vector(-54.508, 40.179, -10.866)), Array(Vector(69.418, 42.103,
			-17.712)), Array(Vector(9.382, 38.635, -81.276)), Array(Vector(9.382, 38.635, -81.276)), Array(Vector(9.756, 30.753, -41.395)),
			Array(Vector(30.313, 28.237, -16.925)), Array(Vector(73.985, 26.198, -109.338)), Array(Vector(-21.030, 35.127, 88.381)), Array(
			Vector(-21.030, 35.127, 88.381)), Array(Vector(54.798, 40.964, -82.780)), Array(Vector(54.798, 40.964, -82.780)), Array(Vector(
			-0.355, 28.167, -22.396)), Array(Vector(-56.869, 24.061, -32.132)), Array(Vector(-56.869, 24.061, -32.132)), Array(Vector(
			16.965, 26.541, -7.130)), Array(Vector(51.885, 37.172, -113.654)), Array(Vector(-329.934, 56.136, 149.839)), Array(Vector(
			322.988, 15.716, -37.732)), Array(Vector(-223.895, 34.756, 0.890)), Array(Vector(-5.808, 324.398, 282.523)), Array(Vector(
			-5.808, 324.398, 282.523)), Array(Vector(-0.414, 156.197, 148.681)), Array(Vector(-0.414, 156.197, 148.681)), Array(Vector(
			-0.381, 53.736, -33.335)), Array(Vector(-0.381, 53.736, -33.335)), Array(Vector(83.100, 178.926, 0.593)), Array(Vector(-49.803,
			63.290, -0.413)), Array(Vector(-184.659, -38.730, -0.783)), Array(Vector(150.125, 30.619, 251.966)), Array(Vector(134.888,
			36.760, -240.736)), Array(Vector(-195.549, 60.350, -0.098)), Array(Vector(-9.622, 44.751, 5.635)), Array(Vector(-9.622, 44.751,
			5.635)), Array(Vector(-100.828, 76.566, 60.021)), Array(Vector(200.414, 82.412, 78.813)), Array(Vector(200.414, 82.412,
			78.813)), Array(Vector(-36.319, 32.305, 0.472)), Array(Vector(9.678, 28.313, 13.400)), Array(Vector(-49.147, 22.344, 76.844)),
			Array(Vector(54.948, 50.769, 3.930)), Array(Vector(322.988, 40, -37.732), Vector(27.711, 100, -161.298), Vector(-223.895, 50,
			0.890)), Array(Vector(51.885, 37.172, -113.654), Vector(-329.934, 56.136, 149.839), Vector(227.210, 43.353, 252.640)), Array(
			Vector(-5.808, 324.398, 282.523), Vector(-0.414, 156.197, 148.681), Vector(-0.381, 53.736, -33.335)), Array(Vector(-5.808,
			324.398, 282.523), Vector(-0.414, 156.197, 148.681), Vector(-0.381, 53.736, -33.335)), Array(Vector(83.100, 178.926, 0.593),
			Vector(-49.803, 63.290, -0.413), Vector(-184.659, -38.730, -0.783)), Array(Vector(150.125, 30.619, 251.966), Vector(134.888,
			36.760, -240.736), Vector(-195.549, 60.350, -0.098)), Array(Vector(17.808, 35.955, 17.505)));
		For Player Variable(Global.MsDestructo, ArrayIterator, 0, Count Of(Global.MsDestructo.MapVectorArray[Index Of Array Value(First Of(
			Global.MsDestructo.MapVectorArray), Current Map)]), 1);
			Start Forcing Player Position(Global.MsDestructo, Global.MsDestructo.MapVectorArray[Index Of Array Value(
				Global.MsDestructo.MapVectorArray, Current Map)][Global.MsDestructo.ArrayIterator], True);
		...
		Global.MsDestructo.MapVectorArray = 0;
