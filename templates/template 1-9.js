    ...
    settings
{
	main
	{
		...
		Mode Name: "Genji Parkour - 源氏跑酷 - v1_9"
	}
	lobby
	{
...
subroutines {
    ...
    12: StartPauseTimer
    13: StopPauseTimer
    14: editupdateselectedids
    15: EditorSelectLast
    16: TriggerOnFailSuccesReset
    17: RebuildBounceOrbs
    18: RebuildKillOrbs
    19: RebuildPortals
    20: UpdateCache
    21: checkpointFailReset
}
...
rule ("SUB | Rebuild Bounce Orbs") {
    ...
    actions {
        ...
        For Global Variable(NANBA, 0, Count Of(Global.pinballnumber), 1);
            ...
            "wait()"
            If(Compare(Modulo(Global.NANBA, 5), ==, 0));
...
rule ("Subroutine RebuildPortals") {
    event {
        Subroutine;
        RebuildPortals;
    }
...
rule ("SUB | Update Effect Cache") {
    ...
    actions {
        ...
        Set Player Variable(Event Player, PortalStart_Cache, Filtered Array(Global.CustomPortalStart, Or(Compare(Value In Array(Global.CustomPortalCP, Current Array Index), ==, (Event Player).A), Compare(Value In Array(Global.CustomPortalCP, Current Array Index), ==, 999))));
        Set Player Variable(Event Player, PortalEnd_cache, Filtered Array(Global.CustomPortalEndpoint, Or(Compare(Value In Array(Global.CustomPortalCP, Current Array Index), ==, (Event Player).A), Compare(Value In Array(Global.CustomPortalCP, Current Array Index), ==, 999))));
        Set Player Variable(Event Player, banstring, Custom String("", Null, Null, Null));
        Wait(0.016, Ignore Condition);
        If(Workshop Setting Toggle(Custom String("Ban (applies to all levels)\n封禁(应用于所有关卡)", Null, Null, Null), Custom String("ban Multiclim - 封禁蹭留", Null, Null, Null), False, 1));
        ...
        End;
        If(Workshop Setting Toggle(Custom String("Ban (applies to all levels)\n封禁(应用于所有关卡)", Null, Null, Null), Custom String("ban Createbhop - 封禁卡小", Null, Null, Null), False, 2));
        ...
        End;
        If(Workshop Setting Toggle(Custom String("Ban (applies to all levels)\n封禁(应用于所有关卡)", Null, Null, Null), Custom String("ban Deathbhop - 封禁死小", Null, Null, Null), False, 3));
        ...
        End;
        If(Workshop Setting Toggle(Custom String("Ban (applies to all levels)\n封禁(应用于所有关卡)", Null, Null, Null), Custom String("ban Emote Savehop - 封禁表情爬", Null, Null, Null), False, 4));
        ...
        End;
        If(Workshop Setting Toggle(Custom String("Ban (applies to all levels)\n封禁(应用于所有关卡)", Null, Null, Null), Custom String("ban Wallclimb - 封禁爬墙", Null, Null, Null), False, 5));
        ...
        End;
        If(Workshop Setting Toggle(Custom String("Ban (applies to all levels)\n封禁(应用于所有关卡)", Null, Null, Null), Custom String("require bhop available - 留小跳进点 ", Null, Null, Null), False, 5));
            Set Player Variable(Event Player, ban_bhop, True);
...
rule ("Editor | Clear Excess Data to Save Map") {
    ...
    actions {
        ...
        Set Player Variable(Event Player, LockEditor, True);
        ...
        Set Player Variable(Event Player, savemaphud, Null);
        Set Player Variable(Event Player, LockEditor, False);
    }
}
...
rule ("Editor | Hud and Effects") {
    ...
    actions {
        ...
        Set Global Variable(EditorMoveItem, False);
        ...
        End;
        Set Global Variable(CustomPortalStart, If-Then-Else(Compare(Count Of(Global.CustomPortalStart), >, 0), Global.CustomPortalStart, Empty Array));
        Set Global Variable(CustomPortalEndpoint, If-Then-Else(Compare(Count Of(Global.CustomPortalEndpoint), >, 0), Global.CustomPortalEndpoint, Empty Array));
        Set Global Variable(CustomPortalCP, If-Then-Else(Compare(Count Of(Global.CustomPortalCP), >, 0), Global.CustomPortalCP, Empty Array));
        "huds ==========================================================================================================================================================================\r\n restart without leaderboard (old one deleted)"
        Create HUD Text(If-Then-Else((Local Player).K, Local Player, Null), Null, Null, If-Then-Else(Or(Compare(String("Capture", Null, Null, Null), ==, Custom String("捕捉", Null, Null, Null)), Compare(Custom String("{0}", Color(Rose), Null, Null), ==, Custom String("玫红", Null, Null, Null))), Custom String("{0}+{1}+{2} | 重新开始", Input Binding String(Button(Crouch)), Input Binding String(Button(Ability 2)), Input Binding String(Button(Interact))), Custom String("{0}+{1}+{2} | Restart", Input Binding String(Button(Crouch)), Input Binding String(Button(Ability 2)), Input Binding String(Button(Interact)))), Right, -160, Null, Null, Color(White), Visible To and String, Default Visibility);
        Create HUD Text(If-Then-Else((Host Player).K, Host Player, Null), Null, Null, Custom String("{0}", If-Then-Else(Or(Compare(String("Capture", Null, Null, Null), ==, Custom String("捕捉", Null, Null, Null)), Compare(Custom String("{0}", Color(Rose), Null, Null), ==, Custom String("玫红", Null, Null, Null))), If-Then-Else(Compare(Global.EditorMoveItem, ==, False), Value In Array(Array(Custom String("{0} + {1} | 新建检查点\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Primary Fire)), Custom String("{0} + {1} | 删除选中的检查点\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Secondary Fire)), Custom String("{0} + {1} | 移除/新建传送点\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Reload)), Custom String("{0} + {1} | 检查点碰撞模型\n{2} (长按) | 移动检查点", Input Binding String(Button(Interact)), Input Binding String(Button(Ability 1)), Input Binding String(Button(Ability 2)))))), Custom String("{0} + {1} | 新建击杀球\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Primary Fire)), Custom String("{0} + {1} (长按) | 在准心位置新建\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Primary Fire)), Custom String("{0} + {1} | 删除选中的击杀球\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Secondary Fire)), Custom String("{0} + {1} | 选择上一个击杀球\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Crouch)), Custom String("{0} + {1} | 选择下一个击杀球\n\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Jump)), Custom String("{0} + {1} | 增大击杀球尺寸\n{2}", Input Binding String(Button(Ability 2)), Input Binding String(Button(Jump)), Custom String("{0} + {1} | 减小击杀球尺寸\n{0} + {2} | 移动选中的实体", Input Binding String(Button(Ability 2)), Input Binding String(Button(Crouch)), Input Binding String(Button(Primary Fire))))))))), Custom String("{0} + {1} | 新建弹球\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Primary Fire)), Custom String("{0} + {1} (长按) | 在准心位置新建\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Primary Fire)), Custom String("{0} + {1} | 选择上一个弹球\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Crouch)), Custom String("{0} + {1} | 选择下一个弹球\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Jump)), Custom String("{0} + {1} | 删除选中的弹球\n\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Secondary Fire)), Custom String("{0} + {1} | 增加弹球弹力\n{2}", Input Binding String(Button(Ability 2)), Input Binding String(Button(Jump)), Custom String("{0} + {1} | 减少弹球弹力\n{0} + {2} | 移动选中的实体", Input Binding String(Button(Ability 2)), Input Binding String(Button(Crouch)), Input Binding String(Button(Primary Fire))))))))), Custom String("{0} + {1} | 蹭留\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Primary Fire)), Custom String("{0} + {1} | 卡小\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Secondary Fire)), Custom String("{0} + {1} | 爬墙\n\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Jump)), Custom String("{0} + {1} | 死小\n{2}", Input Binding String(Button(Ability 2)), Input Binding String(Button(Primary Fire)), Custom String("{0} + {1} | 表情留小\n{0} + {2} |  留小跳进点", Input Binding String(Button(Ability 2)), Input Binding String(Button(Secondary Fire)), Input Binding String(Button(Jump))))))), Custom String("{0} + {1} | 新建传送门\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Primary Fire)), Custom String("{0} + {1} (长按) | 在准心位置新建\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Primary Fire)), Custom String("{0} + {1} | 删除选中的传送门\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Secondary Fire)), Custom String("{0} + {1} | 选择下一个传送门\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Jump)), Custom String("{0} + {1} | 选择上一个传送门\n\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Crouch)), Custom String("{0} + {1} | 移动选中的实体\n{0} + {2} | 应用到当前/全部检查点(开关)", Input Binding String(Button(Ability 2)), Input Binding String(Button(Primary Fire)), Input Binding String(Button(Jump))))))))), (Host Player).E), Custom String("方向键 | 移动实体 \n{0} | 向上移动 \n{1} | 向下移动 \n{2}", Input Binding String(Button(Jump)), Input Binding String(Button(Crouch)), Custom String("{0} | 放置实体 \n{1} (长按) | 移动更快", Input Binding String(Button(Primary Fire)), Input Binding String(Button(Secondary Fire)), Null))), If-Then-Else(Compare(Global.EditorMoveItem, ==, False), Value In Array(Array(Custom String("{0} + {1} | Create New\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Primary Fire)), Custom String("{0} + {1} | Delete selected\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Secondary Fire)), Custom String("{0} + {1} | Remove/Add teleport\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Reload)), Custom String("{0} + {1} | Toggle Hitbox\n\n{2} (hold) | Move", Input Binding String(Button(Interact)), Input Binding String(Button(Ability 1)), Input Binding String(Button(Ability 2)))))), Custom String("{0} + {1} | Create new\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Primary Fire)), Custom String("{0} + {1} (hold)| raycast new\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Primary Fire)), Custom String("{0} + {1} | Delete selected\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Secondary Fire)), Custom String("{0} + {1} | Select previous\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Crouch)), Custom String("{0} + {1} | Select next\n\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Jump)), Custom String("{0} + {1} | Increase size\n{2}", Input Binding String(Button(Ability 2)), Input Binding String(Button(Jump)), Custom String("{0} + {1} | Decrease size\n{0} + {2} | Move\n", Input Binding String(Button(Ability 2)), Input Binding String(Button(Crouch)), Input Binding String(Button(Primary Fire))))))))), Custom String("{0} + {1} | Create new\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Primary Fire)), Custom String("{0} + {1} (hold)| raycast new\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Primary Fire)), Custom String("{0} + {1} | Select previous\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Crouch)), Custom String("{0} + {1} | Select next\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Jump)), Custom String("{0} + {1} | Delete selected \n\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Secondary Fire)), Custom String("{0} + {1} | Increase strength\n{2}", Input Binding String(Button(Ability 2)), Input Binding String(Button(Jump)), Custom String("{0} + {1} | Decrease strength\n{0} + {2} | Move\n", Input Binding String(Button(Ability 2)), Input Binding String(Button(Crouch)), Input Binding String(Button(Primary Fire))))))))), Custom String("{0} + {1} | multiclimb\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Primary Fire)), Custom String("{0} + {1} | createbhop\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Secondary Fire)), Custom String("{0} + {1} | wallclimb\n\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Jump)), Custom String("{0} + {1} | deadhop\n{2}", Input Binding String(Button(Ability 2)), Input Binding String(Button(Primary Fire)), Custom String("{0} + {1} | emote\n{0} + {2} | require bhop\n", Input Binding String(Button(Ability 2)), Input Binding String(Button(Secondary Fire)), Input Binding String(Button(Jump))))))), Custom String("{0} + {1} | create new\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Primary Fire)), Custom String("{0} + {1} (hold)| raycast new\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Primary Fire)), Custom String("{0} + {1} | delete\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Secondary Fire)), Custom String("{0} + {1} | select next\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Jump)), Custom String("{0} + {1} | select previous\n\n{2}", Input Binding String(Button(Interact)), Input Binding String(Button(Crouch)), Custom String("{0} + {1} | move\n{0} + {2} | cp/map (toggle)\n", Input Binding String(Button(Ability 2)), Input Binding String(Button(Primary Fire)), Input Binding String(Button(Jump))))))))), (Host Player).E), Custom String("walk | move selected \n{0} | move up\n{1} | move down\n{2}", Input Binding String(Button(Jump)), Input Binding String(Button(Crouch)), Custom String("{0} (hold) | move faster\n{1} | confirm placement", Input Binding String(Button(Secondary Fire)), Input Binding String(Button(Primary Fire)), Null)))), Null, Null), Right, -148, Null, Null, Color(Yellow), Visible To and String, Default Visibility);
        Create HUD Text(If-Then-Else((Host Player).K, Host Player, Null), Null, Null, If-Then-Else(Or(Compare(String("Capture", Null, Null, Null), ==, Custom String("捕捉", Null, Null, Null)), Compare(Custom String("{0}", Color(Rose), Null, Null), ==, Custom String("玫红", Null, Null, Null))), Custom String(" \n{0} + {1} | 下一个检查点\n{2}", Input Binding String(Button(Crouch)), Input Binding String(Button(Primary Fire)), Custom String("{0} + {1} | 上一个检查点\n{2} 长按 | 飞行\n", Input Binding String(Button(Crouch)), Input Binding String(Button(Secondary Fire)), Input Binding String(Button(Ability 1)))), Custom String(" \n{0} + {1} | Next checkpoint\n{2}", Input Binding String(Button(Crouch)), Input Binding String(Button(Primary Fire)), Custom String("{0} + {1} | Prev checkpoint\n{2} (hold)| Fly\n", Input Binding String(Button(Crouch)), Input Binding String(Button(Secondary Fire)), Input Binding String(Button(Ability 1))))), Right, -149, Null, Null, If-Then-Else((Host Player).K, Color(Green), Color(Orange)), Visible To String and Color, Default Visibility);
        ...
        Create HUD Text(If-Then-Else((Local Player).savemaphud, Null, Local Player), If-Then-Else(Or(Compare(String("Capture", Null, Null, Null), ==, Custom String("捕捉", Null, Null, Null)), Compare(Custom String("{0}", Color(Rose), Null, Null), ==, Custom String("玫红", Null, Null, Null))), If-Then-Else(Is Button Held(Host Player, Button(Melee)), Custom String("{0} 检查点模式 \n{1} 击杀球模式 \n{2}", If-Then-Else(Compare((Host Player).E, ==, 0), Icon String(Arrow: Right), Custom String("     ", Null, Null, Null)), If-Then-Else(Compare((Host Player).E, ==, 1), Icon String(Arrow: Right), Custom String("     ", Null, Null, Null)), Custom String("{0} 弹球模式 \n{1} 封禁(单关) \n{2} 自定义传送门 ", If-Then-Else(Compare((Host Player).E, ==, 2), Icon String(Arrow: Right), Custom String("     ", Null, Null, Null)), If-Then-Else(Compare((Host Player).E, ==, 3), Icon String(Arrow: Right), Custom String("     ", Null, Null, Null)), If-Then-Else(Compare((Host Player).E, ==, 4), Icon String(Arrow: Right), Custom String("     ", Null, Null, Null)))), If-Then-Else(Compare(Local Player, ==, Host Player), Custom String(" {0} {1} ", Value In Array(Array(Icon String(Flag), Icon String(Skull), Icon String(Moon), Icon String(Stop), Icon String(Spiral)), (Host Player).E), Value In Array(Array(Custom String("检查点模式", Null, Null, Null), Custom String("击杀球模式", Null, Null, Null), Custom String("弹球模式", Null, Null, Null), Custom String("封禁(单关)", Null, Null, Null), Custom String("自定义传送门", Null, Null, Null)), (Host Player).E), Null), Custom String(" {0} 源氏 编辑者 {0} ", Icon String(Bolt), Null, Null))), If-Then-Else(Is Button Held(Host Player, Button(Melee)), Custom String("{0} Checkpoints \n{1} Kills orbs \n{2}", If-Then-Else(Compare((Host Player).E, ==, 0), Icon String(Arrow: Right), Custom String("     ", Null, Null, Null)), If-Then-Else(Compare((Host Player).E, ==, 1), Icon String(Arrow: Right), Custom String("     ", Null, Null, Null)), Custom String("{0} Bounce Orb \n{1} checkpoint bans\n{2} portals ", If-Then-Else(Compare((Host Player).E, ==, 2), Icon String(Arrow: Right), Custom String("     ", Null, Null, Null)), If-Then-Else(Compare((Host Player).E, ==, 3), Icon String(Arrow: Right), Custom String("     ", Null, Null, Null)), If-Then-Else(Compare((Host Player).E, ==, 4), Icon String(Arrow: Right), Custom String("     ", Null, Null, Null)))), If-Then-Else(Compare(Local Player, ==, Host Player), Custom String(" {0} {1} ", Value In Array(Array(Icon String(Flag), Icon String(Skull), Icon String(Moon), Icon String(Stop), Icon String(Spiral)), (Host Player).E), Value In Array(Array(Custom String("Checkpoints", Null, Null, Null), Custom String("Kill orbs", Null, Null, Null), Custom String("Function Orb", Null, Null, Null), Custom String("checkpoint bans", Null, Null, Null), Custom String("portals", Null, Null, Null)), (Host Player).E), Null), Custom String(" {0} Genji editor {0} ", Icon String(Bolt), Null, Null)))), Null, Null, Top, -175, Color(Blue), Color(Blue), Color(Blue), Visible To and String, Default Visibility);
        Create HUD Text(All Players(All Teams), Null, If-Then-Else(Or(Compare(String("Capture", Null, Null, Null), ==, Custom String("捕捉", Null, Null, Null)), Compare(Custom String("{0}", Color(Rose), Null, Null), ==, Custom String("玫红", Null, Null, Null))), If-Then-Else(Compare(Local Player, ==, Host Player), Custom String("{0} + 射击 | 切换作图模式", Input Binding String(Button(Melee)), Null, Null), Custom String("房主/编辑者 {0}", Host Player, Null, Null)), If-Then-Else(Compare(Local Player, ==, Host Player), Custom String("{0} + shoot | change mode", Input Binding String(Button(Melee)), Null, Null), Custom String("Current host/editor: {0}", Host Player, Null, Null))), Null, Top, -176, Color(Sky Blue), If-Then-Else((Local Player).LockEditor, Color(Gray), Color(White)), Color(White), Visible To String and Color, Default Visibility);
        Create HUD Text(If-Then-Else(And((Host Player).K, Or(Compare((Host Player).E, ==, 0), And(Compare((Host Player).E, ==, 2), Compare(Count Of((Host Player).BounceIndex_Cache), >, 0)))), Host Player, Null), Null, Null, If-Then-Else(Or(Compare(String("Capture", Null, Null, Null), ==, Custom String("捕捉", Null, Null, Null)), Compare(Custom String("{0}", Color(Rose), Null, Null), ==, Custom String("玫红", Null, Null, Null))), Custom String("{0} + {1} | {2}", Input Binding String(Button(Ultimate)), Input Binding String(Button(Primary Fire)), Custom String("{0} {1} | {2}", If-Then-Else(Compare((Host Player).E, ==, 0), Custom String("检查点给刀", Null, Null, Null), Custom String("弹球给刀", Null, Null, Null)), Ability Icon String(Hero(Genji), Button(Ultimate)), If-Then-Else(Compare((Host Player).E, ==, 2), If-Then-Else(Compare(Value In Array(Global.TQ5, Global.EditSelected), !=, 0), Custom String("启用", Null, Null, Null), Custom String("关闭", Null, Null, Null)), If-Then-Else(Array Contains(Global.Dao, (Host Player).A), Custom String("启用", Null, Null, Null), Custom String("关闭", Null, Null, Null))))), Custom String("{0} + {1} | {2}", Input Binding String(Button(Ultimate)), Input Binding String(Button(Primary Fire)), Custom String("{0} give ult {1} | {2}", If-Then-Else(Compare((Host Player).E, ==, 0), Custom String("Level", Null, Null, Null), Custom String("Orb", Null, Null, Null)), Ability Icon String(Hero(Genji), Button(Ultimate)), If-Then-Else(Compare((Host Player).E, ==, 2), If-Then-Else(Compare(Value In Array(Global.TQ5, Global.EditSelected), !=, 0), Custom String("on", Null, Null, Null), Custom String("off", Null, Null, Null)), If-Then-Else(Array Contains(Global.Dao, (Host Player).A), Custom String("on", Null, Null, Null), Custom String("off", Null, Null, Null)))))), Left, -190, Null, Null, If-Then-Else(And(Compare(Value In Array(Global.TQ5, Global.EditSelected), !=, 0), Compare((Host Player).E, ==, 2)), Color(Green), If-Then-Else(And(Array Contains(Global.Dao, (Host Player).A), Compare((Host Player).E, ==, 0)), Color(Green), Color(Orange))), Visible To String and Color, Default Visibility);
        Create HUD Text(If-Then-Else(And((Host Player).K, Or(Compare((Host Player).E, ==, 0), And(Compare((Host Player).E, ==, 2), Compare(Count Of((Host Player).BounceIndex_Cache), >, 0)))), Host Player, Null), Null, Null, If-Then-Else(Or(Compare(String("Capture", Null, Null, Null), ==, Custom String("捕捉", Null, Null, Null)), Compare(Custom String("{0}", Color(Rose), Null, Null), ==, Custom String("玫红", Null, Null, Null))), Custom String("{0} + {1} | {2}", Input Binding String(Button(Ultimate)), Input Binding String(Button(Secondary Fire)), Custom String("{0} {1} | {2}", If-Then-Else(Compare((Host Player).E, ==, 0), Custom String("检查点给Shift", Null, Null, Null), Custom String("弹球给Shift", Null, Null, Null)), Ability Icon String(Hero(Genji), Button(Ability 1)), If-Then-Else(Compare((Host Player).E, ==, 2), If-Then-Else(Compare(Value In Array(Global.TQ6, Global.EditSelected), !=, 0), Custom String("启用", Null, Null, Null), Custom String("关闭", Null, Null, Null)), If-Then-Else(Array Contains(Global.SHIFT, (Host Player).A), Custom String("启用", Null, Null, Null), Custom String("关闭", Null, Null, Null))))), Custom String("{0} + {1} | {2}", Input Binding String(Button(Ultimate)), Input Binding String(Button(Secondary Fire)), Custom String("{0} give dash {1} | {2}", If-Then-Else(Compare((Host Player).E, ==, 0), Custom String("Level", Null, Null, Null), Custom String("Orb", Null, Null, Null)), Ability Icon String(Hero(Genji), Button(Ability 1)), If-Then-Else(Compare((Host Player).E, ==, 2), If-Then-Else(Compare(Value In Array(Global.TQ6, Global.EditSelected), !=, 0), Custom String("on", Null, Null, Null), Custom String("off", Null, Null, Null)), If-Then-Else(Array Contains(Global.SHIFT, (Host Player).A), Custom String("on", Null, Null, Null), Custom String("off", Null, Null, Null)))))), Left, -189, Null, Null, If-Then-Else(And(Compare(Value In Array(Global.TQ6, Global.EditSelected), !=, 0), Compare((Host Player).E, ==, 2)), Color(Green), If-Then-Else(And(Array Contains(Global.SHIFT, (Host Player).A), Compare((Host Player).E, ==, 0)), Color(Green), Color(Orange))), Visible To String and Color, Default Visibility);
        Create HUD Text(If-Then-Else(And(And(Compare((Host Player).E, ==, 2), (Host Player).K), Compare(Count Of((Host Player).BounceIndex_Cache), >, 0)), Host Player, Null), Null, Null, If-Then-Else(Or(Compare(String("Capture", Null, Null, Null), ==, Custom String("捕捉", Null, Null, Null)), Compare(Custom String("{0}", Color(Rose), Null, Null), ==, Custom String("玫红", Null, Null, Null))), Custom String("{0} + {1} |  黄弹球(进点前必须收集) {2}", Input Binding String(Button(Ultimate)), Input Binding String(Button(Ability 2)), Custom String("{0} | {1}\n", Icon String(Asterisk), If-Then-Else(Compare(Value In Array(Global.BounceToggleLock, Global.EditSelected), !=, 0), Custom String("启用", Null, Null, Null), Custom String("关闭", Null, Null, Null)), Null)), Custom String("{0} + {1} | unlocks checkpoint {2}", Input Binding String(Button(Ultimate)), Input Binding String(Button(Ability 2)), Custom String("{0} | {1}\n", Icon String(Asterisk), If-Then-Else(Compare(Value In Array(Global.BounceToggleLock, Global.EditSelected), !=, 0), Custom String("on", Null, Null, Null), Custom String("off", Null, Null, Null)), Null))), Left, -188, Null, Null, If-Then-Else(Compare(Value In Array(Global.BounceToggleLock, Global.EditSelected), !=, 0), Color(Green), Color(Orange)), Visible To String and Color, Default Visibility);
        ...
        Create HUD Text(If-Then-Else((Host Player).K, Host Player, Null), If-Then-Else(Or(Compare(String("Capture", Null, Null, Null), ==, Custom String("捕捉", Null, Null, Null)), Compare(Custom String("{0}", Color(Rose), Null, Null), ==, Custom String("玫红", Null, Null, Null))), If-Then-Else(And(Compare((Host Player).E, ==, 0), Compare(Count Of(Global.A), >, 0)), Custom String("\n 选中的检查点 \n 矢量: {0}{1} \n", Value In Array(Global.A, (Host Player).A), If-Then-Else(Compare(Count Of(Value In Array(Global.A, (Host Player).A)), <, 2), Custom String("", Null, Null, Null), Custom String("\n 传送点: {0}", Value In Array(Value In Array(Global.A, (Host Player).A), 1), Null, Null)), Null), If-Then-Else(And(Compare((Host Player).E, ==, 1), Compare(Count Of((Host Player).KillIndex_Cache), >, 0)), Custom String("\n 选中的击杀球 \n 矢量: {0} \n 半径: {1} \n", Value In Array(Global.H, Global.EditSelected), Value In Array(Global.I, Global.EditSelected), Null), If-Then-Else(And(Compare((Host Player).E, ==, 2), Compare(Count Of((Host Player).BounceIndex_Cache), >, 0)), Custom String("\n 选中的弹球 \n 矢量: {0} \n 弹力: {1} \n ID: {2}", Value In Array(Global.TQ, Global.EditSelected), Value In Array(Global.EditMode, Global.EditSelected), Global.EditSelected), If-Then-Else(Compare((Host Player).E, ==, 3), Custom String("封禁(单关)\n――――――――――――\n {0} 蹭留\n {1} 卡小\n {2}", If-Then-Else(Array Contains(Global.BanMulti, (Host Player).A), Custom String("√", Null, Null, Null), Custom String(" ", Null, Null, Null)), If-Then-Else(Array Contains(Global.BanCreate, (Host Player).A), Custom String("√", Null, Null, Null), Custom String(" ", Null, Null, Null)), Custom String("{0} 爬墙\n {1} 死小\n {2}", If-Then-Else(Array Contains(Global.BanClimb, (Host Player).A), Custom String("√", Null, Null, Null), Custom String(" ", Null, Null, Null)), If-Then-Else(Array Contains(Global.BanDead, (Host Player).A), Custom String("√", Null, Null, Null), Custom String(" ", Null, Null, Null)), Custom String("{0} 表情留小\n――――――――――――\n {1} 留小跳进点\n", If-Then-Else(Array Contains(Global.BanEmote, (Host Player).A), Custom String("√", Null, Null, Null), Custom String(" ", Null, Null, Null)), If-Then-Else(Array Contains(Global.BanBhop, (Host Player).A), Custom String("√", Null, Null, Null), Custom String(" ", Null, Null, Null)), Null))), If-Then-Else(And(And(Compare((Host Player).E, ==, 4), Array Contains(Array((Host Player).A, 999), Value In Array(Global.CustomPortalCP, Global.EditSelected))), Compare(Count Of(Global.CustomPortalCP), >, 0)), Custom String("\n 传送门起点: {0} \n 传送门终点位: {1} \n 传送门 检查点 : {2} \n", Value In Array(Global.CustomPortalStart, Global.EditSelected), Value In Array(Global.CustomPortalEndpoint, Global.EditSelected), If-Then-Else(Compare(Value In Array(Global.CustomPortalCP, Global.EditSelected), ==, 999), Custom String("所有", Null, Null, Null), (Host Player).A)), Custom String("\n   当前无数据选中   \n", Null, Null, Null)))))), If-Then-Else(And(Compare((Host Player).E, ==, 0), Compare(Count Of(Global.A), >, 0)), Custom String("\n Selected Checkpoint \n Vector: {0}{1} \n", Value In Array(Global.A, (Host Player).A), If-Then-Else(Compare(Count Of(Value In Array(Global.A, (Host Player).A)), <, 2), Custom String("", Null, Null, Null), Custom String("\n Teleport: {0}", Value In Array(Value In Array(Global.A, (Host Player).A), 1), Null, Null)), Null), If-Then-Else(And(Compare((Host Player).E, ==, 1), Compare(Count Of((Host Player).KillIndex_Cache), >, 0)), Custom String("\n Selected Kill Orb \n Vector: {0} \n radius: {1} \n", Value In Array(Global.H, Global.EditSelected), Value In Array(Global.I, Global.EditSelected), Null), If-Then-Else(And(Compare((Host Player).E, ==, 2), Compare(Count Of((Host Player).BounceIndex_Cache), >, 0)), Custom String("\n Selected Bounce Orb \n Vector: {0} \n strength: {1} \n ID: {2}", Value In Array(Global.TQ, Global.EditSelected), Value In Array(Global.EditMode, Global.EditSelected), Global.EditSelected), If-Then-Else(Compare((Host Player).E, ==, 3), Custom String(" bans\n――――――――――――\n {0} multi\n {1} create\n {2}", If-Then-Else(Array Contains(Global.BanMulti, (Host Player).A), Custom String("√", Null, Null, Null), Custom String(" ", Null, Null, Null)), If-Then-Else(Array Contains(Global.BanCreate, (Host Player).A), Custom String("√", Null, Null, Null), Custom String(" ", Null, Null, Null)), Custom String("{0} climb\n {1} dead\n {2}", If-Then-Else(Array Contains(Global.BanClimb, (Host Player).A), Custom String("√", Null, Null, Null), Custom String(" ", Null, Null, Null)), If-Then-Else(Array Contains(Global.BanDead, (Host Player).A), Custom String("√", Null, Null, Null), Custom String(" ", Null, Null, Null)), Custom String("{0} emote\n――――――――――――\n {1} require bhop\n", If-Then-Else(Array Contains(Global.BanEmote, (Host Player).A), Custom String("√", Null, Null, Null), Custom String(" ", Null, Null, Null)), If-Then-Else(Array Contains(Global.BanBhop, (Host Player).A), Custom String("√", Null, Null, Null), Custom String(" ", Null, Null, Null)), Null))), If-Then-Else(And(And(Compare((Host Player).E, ==, 4), Array Contains(Array((Host Player).A, 999), Value In Array(Global.CustomPortalCP, Global.EditSelected))), Compare(Count Of(Global.CustomPortalCP), >, 0)), Custom String("\n Start: {0} \n End: {1} \n CP: {2} \n", Value In Array(Global.CustomPortalStart, Global.EditSelected), Value In Array(Global.CustomPortalEndpoint, Global.EditSelected), If-Then-Else(Compare(Value In Array(Global.CustomPortalCP, Global.EditSelected), ==, 999), Custom String("All", Null, Null, Null), (Host Player).A)), Custom String("\n   No data selected   \n", Null, Null, Null))))))), Null, Null, Left, -191, Color(White), Color(Orange), Color(Orange), Visible To and String, Default Visibility);
        Wait(1, Ignore Condition);
        "effects =========================================================================================================================================================================="
        Create In-World Text(If-Then-Else(Compare(Count Of(Global.EditSelectIdArray), >, 0), All Players(All Teams), Null), If-Then-Else(Or(Compare(String("Capture", Null, Null, Null), ==, Custom String("捕捉", Null, Null, Null)), Compare(Custom String("{0}", Color(Rose), Null, Null), ==, Custom String("玫红", Null, Null, Null))), Custom String("选中的实体", Null, Null, Null), Custom String("selected", Null, Null, Null)), If-Then-Else(Compare((Host Player).E, ==, 1), Value In Array(Global.H, Global.EditSelected), If-Then-Else(Compare((Host Player).E, ==, 2), Value In Array(Global.TQ, Global.EditSelected), If-Then-Else(Compare((Host Player).E, ==, 4), Value In Array(Global.CustomPortalStart, Global.EditSelected), Null))), 1.2, Do Not Clip, Visible To Position and String, Color(Orange), Default Visibility);
        Create Icon(If-Then-Else(Compare(Count Of(Global.EditSelectIdArray), >, 0), All Players(All Teams), Null), If-Then-Else(Compare((Host Player).E, ==, 1), Value In Array(Global.H, Global.EditSelected), If-Then-Else(Compare((Host Player).E, ==, 2), Value In Array(Global.TQ, Global.EditSelected), If-Then-Else(Compare((Host Player).E, ==, 4), Value In Array(Global.CustomPortalStart, Global.EditSelected), Null))), Arrow: Down, Visible To and Position, Color(White), True);
        ...
        "portal fx"
        Create Effect(If-Then-Else(And(Compare(Count Of(Global.EditSelectIdArray), >, 0), Compare((Host Player).E, ==, 4)), Host Player, Null), Sparkles, Color(Purple), Value In Array(Global.CustomPortalEndpoint, Global.EditSelected), 0.2, Visible To Position and Radius);
    }
}
...
rule ("Editor |  Fly/Noclip Toggle") {
    ...
    actions {
        ...
        Wait Until(Not(Is Button Held(Event Player, Button(Ability 1))), 1);
        While(And(And(Is Alive(Event Player), Compare((Event Player).flytoggle, !=, Null)), Not(Is Button Held(Event Player, Button(Ability 1)))));
            If(Not(And(Compare(Event Player, ==, Host Player), Global.EditorMoveItem)));
...
rule ("Editor | change mode") {
    ...
    conditions {
        ...
        (Event Player).LockEditor == False;
    }
    actions {
        Set Player Variable(Event Player, LockEditor, True);
        Set Global Variable(EditorMoveItem, False);
        If(Is Button Held(Event Player, Button(Primary Fire)));
            While(And(Is Button Held(Event Player, Button(Melee)), Is Button Held(Event Player, Button(Primary Fire))));
                Set Player Variable(Event Player, E, If-Then-Else(Compare((Event Player).E, >, 0), Subtract((Event Player).E, 1), 4));
                Wait Until(Not(Is Button Held(Event Player, Button(Primary Fire))), 0.1);
                Wait(0.1, Ignore Condition);
            End;
        Else;
            While(And(Is Button Held(Event Player, Button(Melee)), Is Button Held(Event Player, Button(Secondary Fire))));
                Set Player Variable(Event Player, E, Modulo(Add((Event Player).E, 1), 5));
                Wait Until(Not(Is Button Held(Event Player, Button(Secondary Fire))), 0.1);
                Wait(0.1, Ignore Condition);
            End;
        End;
        Wait(0.016, Ignore Condition);
...
rule ("edit update selected id") {
    event {
        Subroutine;
        editupdateselectedids;
    }
    actions {
        If(Compare((Host Player).E, ==, 1));
            Set Global Variable(EditSelectIdArray, Mapped Array(Global.killballnumber, Current Array Index));
            Set Global Variable(EditSelectIdArray, Filtered Array(Global.EditSelectIdArray, Compare(Value In Array(Global.killballnumber, Current Array Element), ==, (Host Player).A)));
        Else If(Compare((Host Player).E, ==, 2));
            Set Global Variable(EditSelectIdArray, Mapped Array(Global.pinballnumber, Current Array Index));
            Set Global Variable(EditSelectIdArray, Filtered Array(Global.EditSelectIdArray, Compare(Value In Array(Global.pinballnumber, Current Array Element), ==, (Host Player).A)));
        Else If(Compare((Host Player).E, ==, 4));
            Set Global Variable(EditSelectIdArray, Mapped Array(Global.CustomPortalCP, Current Array Index));
            Set Global Variable(EditSelectIdArray, Filtered Array(Global.EditSelectIdArray, Or(Compare(Value In Array(Global.CustomPortalCP, Current Array Element), ==, 999), Compare(Value In Array(Global.CustomPortalCP, Current Array Element), ==, (Host Player).A))));
        Else;
...
rule ("Editor | create cp/orb") {
    ...
    conditions {
        ...
        (Event Player).LockEditor == False;
        Array Contains(Array(0, 1, 2, 4), (Host Player).E) == True;
    }
    actions {
        Set Player Variable(Event Player, LockEditor, True);
        If(Compare((Event Player).E, ==, 0));
            ...
            Else;
                ...
                Set Global Variable(CustomPortalCP, Mapped Array(Global.CustomPortalCP, Add(Current Array Element, If-Then-Else(Compare(Current Array Element, >=, (Host Player).A), 1, 0))));
                ...
                Set Global Variable(BanMulti, Mapped Array(Global.BanMulti, Add(Current Array Element, If-Then-Else(Compare(Current Array Element, >=, (Host Player).A), 1, 0))));
                Set Global Variable(BanCreate, Mapped Array(Global.BanCreate, Add(Current Array Element, If-Then-Else(Compare(Current Array Element, >=, (Host Player).A), 1, 0))));
                Set Global Variable(BanDead, Mapped Array(Global.BanDead, Add(Current Array Element, If-Then-Else(Compare(Current Array Element, >=, (Host Player).A), 1, 0))));
                Set Global Variable(BanEmote, Mapped Array(Global.BanEmote, Add(Current Array Element, If-Then-Else(Compare(Current Array Element, >=, (Host Player).A), 1, 0))));
                Set Global Variable(BanClimb, Mapped Array(Global.BanClimb, Add(Current Array Element, If-Then-Else(Compare(Current Array Element, >=, (Host Player).A), 1, 0))));
                Set Global Variable(BanBhop, Mapped Array(Global.BanBhop, Add(Current Array Element, If-Then-Else(Compare(Current Array Element, >=, (Host Player).A), 1, 0))));
            End;
        ...
        Else If(Compare(Count Of(Global.A), <, 1));
        ...
        Else If(Compare((Event Player).E, ==, 1));
            ...
            Call Subroutine(editupdateselectedids);
            Call Subroutine(EditorSelectLast);
            Create Effect(Filtered Array(All Players(All Teams), Compare((Current Array Element).A, ==, Value In Array(Global.killballnumber, Evaluate Once(Global.EditSelected)))), Sphere, Color(Blue), Value In Array(Global.H, Evaluate Once(Global.EditSelected)), Value In Array(Global.I, Evaluate Once(Global.EditSelected)), Visible To Position and Radius);
            Modify Global Variable(K, Append To Array, Last Created Entity);
            ...
            Wait Until(Not(And(Is Button Held(Event Player, Button(Interact)), Is Button Held(Event Player, Button(Primary Fire)))), 1);
            "editupdateselectedids() # to arrow during the placement properly"
            While(And(Is Button Held(Event Player, Button(Interact)), Is Button Held(Event Player, Button(Primary Fire))));
                Set Global Variable At Index(H, Global.EditSelected, Ray Cast Hit Position(Eye Position(Event Player), Add(Eye Position(Event Player), Multiply(Facing Direction Of(Event Player), 8)), Null, Null, False));
                Wait(0.016, Ignore Condition);
            End;
        Else If(Compare((Event Player).E, ==, 2));
            ...
            Call Subroutine(editupdateselectedids);
            Call Subroutine(EditorSelectLast);
            Create Effect(Filtered Array(Append To Array(All Players(All Teams), Null), And(Compare((Current Array Element).A, ==, Value In Array(Global.pinballnumber, Evaluate Once(Global.EditSelected))), Not(Array Contains((Current Array Element).LockCollected, Value In Array(Global.TQ, Evaluate Once(Global.EditSelected)))))), Orb, If-Then-Else(Value In Array(Global.BounceToggleLock, Evaluate Once(Global.EditSelected)), Color(Orange), Color(Green)), Value In Array(Global.TQ, Evaluate Once(Global.EditSelected)), 1, Visible To Position Radius and Color);
            Modify Global Variable(TQ2, Append To Array, Last Created Entity);
            ...
            Wait Until(Not(And(Is Button Held(Event Player, Button(Interact)), Is Button Held(Event Player, Button(Primary Fire)))), 1);
            While(And(Is Button Held(Event Player, Button(Interact)), Is Button Held(Event Player, Button(Primary Fire))));
                Set Global Variable At Index(TQ, Global.EditSelected, Ray Cast Hit Position(Eye Position(Event Player), Add(Eye Position(Event Player), Multiply(Facing Direction Of(Event Player), 7)), Null, Null, False));
                Wait(0.016, Ignore Condition);
            End;
        Else If(Compare((Event Player).E, ==, 4));
            Modify Global Variable(CustomPortalStart, Append To Array, Position Of(Event Player));
            Modify Global Variable(CustomPortalEndpoint, Append To Array, Add(Position Of(Event Player), Vector(0, 10, 0)));
            Modify Global Variable(CustomPortalCP, Append To Array, (Event Player).A);
            Call Subroutine(editupdateselectedids);
            Call Subroutine(EditorSelectLast);
            Create Effect(Filtered Array(All Players(All Teams), Or(Compare((Current Array Element).A, ==, Value In Array(Global.CustomPortalCP, Evaluate Once(Global.EditSelected))), Compare(Value In Array(Global.CustomPortalCP, Evaluate Once(Global.EditSelected)), ==, 999))), Good Aura, Color(White), Value In Array(Global.CustomPortalStart, Evaluate Once(Global.EditSelected)), 0.6, Visible To Position Radius and Color);
            Modify Global Variable(PortalEffects, Append To Array, Last Created Entity);
            Set Global Variable(EditSelected, Subtract(Count Of(Global.CustomPortalStart), 1));
            Wait Until(Not(And(Is Button Held(Event Player, Button(Interact)), Is Button Held(Event Player, Button(Primary Fire)))), 1);
            "editupdateselectedids()"
            While(And(Is Button Held(Event Player, Button(Interact)), Is Button Held(Event Player, Button(Primary Fire))));
                Set Global Variable At Index(CustomPortalStart, Global.EditSelected, Ray Cast Hit Position(Eye Position(Event Player), Add(Eye Position(Event Player), Multiply(Facing Direction Of(Event Player), 6)), Null, Null, False));
                Wait(0.016, Ignore Condition);
            End;
            Big Message(All Players(All Teams), If-Then-Else(Or(Compare(String("Capture", Null, Null, Null), ==, Custom String("捕捉", Null, Null, Null)), Compare(Custom String("{0}", Color(Rose), Null, Null), ==, Custom String("玫红", Null, Null, Null))), Custom String("新传送门已创建!\n生效于当前检查点", Null, Null, Null), Custom String("Portal created \r\nIt's only valid for checkpoint", Null, Null, Null)));
...
rule ("Editor | delete cp/orb/portal") {
    ...
    actions {
        ...
        If(And(Compare((Event Player).E, ==, 0), Compare(Count Of(Global.A), >, 0)));
            ...
            Set Player Variable(Event Player, Temp, Empty Array);
            Set Player Variable(Event Player, Temp, Filtered Array(Mapped Array(Global.CustomPortalCP, If-Then-Else(Compare(Current Array Element, ==, (Host Player).A), Current Array Index, -1)), Compare(Current Array Element, >=, 0)));
            For Global Variable(NANBA, 0, Count Of((Event Player).Temp), 1);
                Destroy Effect(Value In Array(Global.PortalEffects, Value In Array((Event Player).Temp, Global.NANBA)));
                Modify Global Variable(PortalEffects, Remove From Array By Index, Value In Array((Event Player).Temp, Global.NANBA));
                Wait(0.016, Ignore Condition);
            End;
            "Remove specified checkpoint"
            Set Global Variable(CustomPortalCP, Remove From Array(Global.CustomPortalCP, (Host Player).A));
            "Decrement checkpoints after removed one"
            Set Global Variable(CustomPortalCP, Mapped Array(Global.CustomPortalCP, Subtract(Current Array Element, If-Then-Else(Compare(Current Array Element, >, (Host Player).A), 1, 0))));
            "Remove Radii at Checkpoint indexes (temp)"
            Set Global Variable(CustomPortalStart, Filtered Array(Global.CustomPortalStart, Not(Array Contains((Event Player).Temp, Current Array Index))));
            Set Global Variable(CustomPortalEndpoint, Filtered Array(Global.CustomPortalEndpoint, Not(Array Contains((Event Player).Temp, Current Array Index))));
            Set Player Variable(Event Player, Temp, Empty Array);
            ...
            Set Global Variable(BanMulti, Remove From Array(Global.BanMulti, (Host Player).A));
            Set Global Variable(BanMulti, Mapped Array(Global.BanMulti, Subtract(Current Array Element, If-Then-Else(Compare(Current Array Element, >=, (Host Player).A), 1, 0))));
            Set Global Variable(BanCreate, Remove From Array(Global.BanCreate, (Host Player).A));
            Set Global Variable(BanCreate, Mapped Array(Global.BanCreate, Subtract(Current Array Element, If-Then-Else(Compare(Current Array Element, >=, (Host Player).A), 1, 0))));
            Set Global Variable(BanDead, Remove From Array(Global.BanDead, (Host Player).A));
            Set Global Variable(BanDead, Mapped Array(Global.BanDead, Subtract(Current Array Element, If-Then-Else(Compare(Current Array Element, >=, (Host Player).A), 1, 0))));
            Set Global Variable(BanEmote, Remove From Array(Global.BanEmote, (Host Player).A));
            Set Global Variable(BanEmote, Mapped Array(Global.BanEmote, Subtract(Current Array Element, If-Then-Else(Compare(Current Array Element, >=, (Host Player).A), 1, 0))));
            Set Global Variable(BanClimb, Remove From Array(Global.BanClimb, (Host Player).A));
            Set Global Variable(BanClimb, Mapped Array(Global.BanClimb, Subtract(Current Array Element, If-Then-Else(Compare(Current Array Element, >=, (Host Player).A), 1, 0))));
            Set Global Variable(BanBhop, Remove From Array(Global.BanBhop, (Host Player).A));
            Set Global Variable(BanBhop, Mapped Array(Global.BanBhop, Subtract(Current Array Element, If-Then-Else(Compare(Current Array Element, >=, (Host Player).A), 1, 0))));
            ...
            If(Or(And(Compare((Host Player).A, <, 1), Compare(Count Of(Global.A), >, 0)), Compare(Global.A, ==, Empty Array)));
        ...
        Else If(And(Compare((Event Player).E, ==, 1), Compare(Count Of(Global.EditSelectIdArray), >, 0)));
            ...
            Call Subroutine(RebuildKillOrbs);
        Else If(And(Compare((Event Player).E, ==, 2), Compare(Count Of(Global.EditSelectIdArray), >, 0)));
            ...
            Call Subroutine(RebuildBounceOrbs);
        Else If(And(Compare((Event Player).E, ==, 4), Compare(Count Of(Global.EditSelectIdArray), >, 0)));
...
rule ("Editor | toggle orb functions") {
    ...
    conditions {
        ...
        Count Of(Global.EditSelectIdArray) > 0;
        (Event Player).LockEditor == False;
    }
    actions {
        Set Player Variable(Event Player, LockEditor, True);
        ...
        Else;
            ...
            Set Global Variable At Index(EditMode, Global.EditSelected, If-Then-Else(Value In Array(Global.BounceToggleLock, Global.EditSelected), 0, 10));
        End;
        Call Subroutine(UpdateCache);
        Set Player Variable(Event Player, LockEditor, False);
        Wait(0.016, Ignore Condition);
...
rule ("Editor | orb radi/strength") {
    ...
    conditions {
        (Host Player).EditorOn != False;
        Event Player == Host Player;
        Array Contains(Array(1, 2), (Event Player).E) == True;
        Count Of(Global.EditSelectIdArray) > 0;
    ...
    actions {
        Set Player Variable(Event Player, LockEditor, True);
        While(And(Is Button Held(Event Player, Button(Ability 2)), Or(Is Button Held(Event Player, Button(Jump)), Is Button Held(Event Player, Button(Crouch)))));
            If(And(Compare((Event Player).E, ==, 2), Compare(Count Of((Event Player).BounceIndex_Cache), >, 0)));
                Modify Global Variable At Index(EditMode, Global.EditSelected, Add, If-Then-Else(Is Button Held(Event Player, Button(Jump)), 0.1, -0.1));
            Else If(And(Compare((Event Player).E, ==, 1), Compare(Count Of((Event Player).KillIndex_Cache), >, 0)));
                Modify Global Variable At Index(I, Global.EditSelected, Add, If-Then-Else(Is Button Held(Event Player, Button(Jump)), 0.1, -0.1));
            End;
            Wait(0.1, Ignore Condition);
        End;
        Call Subroutine(UpdateCache);
...
rule ("Editor | select orb/portal") {
    ...
    conditions {
        (Host Player).EditorOn != False;
        Event Player == Host Player;
        Array Contains(Array(1, 2, 4), (Event Player).E) == True;
        Count Of(Global.EditSelectIdArray) > 0;
        Is Button Held(Event Player, Button(Interact)) == True;
    ...
    actions {
        ...
        If(Is Button Held(Event Player, Button(Crouch)));
            Set Global Variable(EditSelected, If-Then-Else(Compare(Index Of Array Value(Global.EditSelectIdArray, Global.EditSelected), ==, 0), Last Of(Global.EditSelectIdArray), Value In Array(Global.EditSelectIdArray, Subtract(Index Of Array Value(Global.EditSelectIdArray, Global.EditSelected), 1))));
        Else;
            Set Global Variable(EditSelected, If-Then-Else(Compare(Index Of Array Value(Global.EditSelectIdArray, Global.EditSelected), ==, Subtract(Count Of(Global.EditSelectIdArray), 1)), First Of(Global.EditSelectIdArray), Value In Array(Global.EditSelectIdArray, Add(Index Of Array Value(Global.EditSelectIdArray, Global.EditSelected), 1))));
        End;
        Wait(0.016, Ignore Condition);
        Set Player Variable(Event Player, LockEditor, False);
        Wait Until(Or(Not(Is Button Held(Event Player, Button(Interact))), Not(Or(Is Button Held(Event Player, Button(Crouch)), Is Button Held(Event Player, Button(Jump))))), 0.24);
    }
}
...
rule ("Editor | cp add/remove teleport") {
    ...
    actions {
        ...
        Set Player Variable(Host Player, LockEditor, True);
        If(Compare((Host Player).A, ==, 0));
            ...
            Set Player Variable(Host Player, LockEditor, False);
            Abort;
        End;
        "remove"
...
rule ("Editor | moving checkpoint") {
    ...
    actions {
        ...
        While(And(And(Is Button Held(Event Player, Button(Ability 2)), Is Alive(Event Player)), Not(Is Button Held(Event Player, Button(Interact)))));
...
rule ("Editor | toggle bans") {
    ...
    conditions {
        ...
        (Host Player).E == 3;
        Count Of(Global.A) > 0;
        Or(Or(Is Button Held(Event Player, Button(Primary Fire)), Is Button Held(Event Player, Button(Secondary Fire))), Is Button Held(Event Player, Button(Jump))) == True;
        Or(Is Button Held(Event Player, Button(Interact)), Is Button Held(Event Player, Button(Ability 2))) == True;
        (Event Player).LockEditor == False;
    }
    actions {
        Set Player Variable(Event Player, LockEditor, True);
        If(Is Button Held(Event Player, Button(Interact)));
            If(Is Button Held(Event Player, Button(Primary Fire)));
                If(Array Contains(Global.BanMulti, (Event Player).A));
                    Set Global Variable(BanMulti, Remove From Array(Global.BanMulti, (Event Player).A));
                Else;
                    Modify Global Variable(BanMulti, Append To Array, (Event Player).A);
                End;
            Else If(Is Button Held(Event Player, Button(Secondary Fire)));
                If(Array Contains(Global.BanCreate, (Event Player).A));
                    Set Global Variable(BanCreate, Remove From Array(Global.BanCreate, (Event Player).A));
                Else;
                    Modify Global Variable(BanCreate, Append To Array, (Event Player).A);
                End;
            Else;
                If(Array Contains(Global.BanClimb, (Event Player).A));
                    Set Global Variable(BanClimb, Remove From Array(Global.BanClimb, (Event Player).A));
                Else;
                    Modify Global Variable(BanClimb, Append To Array, (Event Player).A);
                End;
            End;
        Else;
            If(Is Button Held(Event Player, Button(Primary Fire)));
                If(Array Contains(Global.BanDead, (Event Player).A));
                    Set Global Variable(BanDead, Remove From Array(Global.BanDead, (Event Player).A));
                Else;
                    Modify Global Variable(BanDead, Append To Array, (Event Player).A);
                End;
            Else If(Is Button Held(Event Player, Button(Secondary Fire)));
                If(Array Contains(Global.BanEmote, (Event Player).A));
                    Set Global Variable(BanEmote, Remove From Array(Global.BanEmote, (Event Player).A));
                Else;
                    Modify Global Variable(BanEmote, Append To Array, (Event Player).A);
                End;
            Else;
                If(Array Contains(Global.BanBhop, (Event Player).A));
                    Set Global Variable(BanBhop, Remove From Array(Global.BanBhop, (Event Player).A));
                Else;
                    Modify Global Variable(BanBhop, Append To Array, (Event Player).A);
                End;
            End;
        End;
...
rule ("Editor | portal cp change") {
    ...
    actions {
        Set Global Variable At Index(CustomPortalCP, Global.EditSelected, If-Then-Else(Compare(Value In Array(Global.CustomPortalCP, Global.EditSelected), ==, 999), (Event Player).A, 999));
        Wait(0.3, Ignore Condition);
    }
}
...
rule ("Editor | move object") {
    ...
    conditions {
        ...
        Event Player == Host Player;
        Array Contains(Array(1, 2, 4), (Event Player).E) == True;
        Or(Global.EditorMoveItem, And(And(Is Button Held(Event Player, Button(Primary Fire)), Is Button Held(Event Player, Button(Ability 2))), Compare((Event Player).LockEditor, ==, False))) == True;
        Count Of(Global.EditSelectIdArray) > 0;
    }
    actions {
        Set Player Variable(Event Player, LockEditor, True);
        Set Global Variable(EditorMoveItem, True);
        If(Compare((Event Player).flytoggle, ==, Null));
        ...
        If(Compare((Event Player).E, ==, 1));
            Start Camera(Event Player, Add(Value In Array(Global.H, Global.EditSelected), Multiply(Facing Direction Of(Event Player), Multiply(Value In Array(Global.I, Global.EditSelected), -1.2))), Value In Array(Global.H, Global.EditSelected), 30);
            While(Not(Is Button Held(Event Player, Button(Primary Fire))));
                Modify Global Variable At Index(H, Global.EditSelected, Add, Multiply(Vector(0, If-Then-Else(Is Button Held(Event Player, Button(Jump)), 0.1, If-Then-Else(Is Button Held(Event Player, Button(Crouch)), -0.1, 0)), 0), If-Then-Else(Is Button Held(Event Player, Button(Secondary Fire)), 1, 0.3)));
                Modify Global Variable At Index(H, Global.EditSelected, Add, World Vector Of(Multiply(Throttle Of(Event Player), If-Then-Else(Is Button Held(Event Player, Button(Secondary Fire)), 0.25, 0.08)), Event Player, Rotation));
                Wait(0.016, Ignore Condition);
            End;
        Else If(Compare((Event Player).E, ==, 2));
            Start Camera(Event Player, Add(Value In Array(Global.TQ, Global.EditSelected), Multiply(Facing Direction Of(Event Player), -4)), Value In Array(Global.TQ, Global.EditSelected), 30);
            While(Not(Is Button Held(Event Player, Button(Primary Fire))));
                Modify Global Variable At Index(TQ, Global.EditSelected, Add, Multiply(Vector(0, If-Then-Else(Is Button Held(Event Player, Button(Jump)), 0.1, If-Then-Else(Is Button Held(Event Player, Button(Crouch)), -0.1, 0)), 0), If-Then-Else(Is Button Held(Event Player, Button(Secondary Fire)), 1, 0.3)));
                Modify Global Variable At Index(TQ, Global.EditSelected, Add, World Vector Of(Multiply(Throttle Of(Event Player), If-Then-Else(Is Button Held(Event Player, Button(Secondary Fire)), 0.25, 0.08)), Event Player, Rotation));
                Wait(0.016, Ignore Condition);
            End;
        Else If(Compare((Event Player).E, ==, 4));
            "move start position"
            Start Camera(Event Player, Add(Value In Array(Global.CustomPortalStart, Global.EditSelected), Multiply(Facing Direction Of(Event Player), -3)), Value In Array(Global.CustomPortalStart, Global.EditSelected), 30);
            While(Not(Is Button Held(Event Player, Button(Primary Fire))));
                Modify Global Variable At Index(CustomPortalStart, Global.EditSelected, Add, Multiply(Vector(0, If-Then-Else(Is Button Held(Event Player, Button(Jump)), 0.1, If-Then-Else(Is Button Held(Event Player, Button(Crouch)), -0.1, 0)), 0), If-Then-Else(Is Button Held(Event Player, Button(Secondary Fire)), 1, 0.3)));
                Modify Global Variable At Index(CustomPortalStart, Global.EditSelected, Add, World Vector Of(Multiply(Throttle Of(Event Player), If-Then-Else(Is Button Held(Event Player, Button(Secondary Fire)), 0.25, 0.08)), Event Player, Rotation));
                Wait(0.016, Ignore Condition);
            End;
            "move destination"
            Start Camera(Event Player, Add(Value In Array(Global.CustomPortalEndpoint, Global.EditSelected), Multiply(Facing Direction Of(Event Player), -3)), Value In Array(Global.CustomPortalEndpoint, Global.EditSelected), 30);
            Wait Until(Not(Is Button Held(Event Player, Button(Primary Fire))), 1);
            While(Not(Is Button Held(Event Player, Button(Primary Fire))));
                Modify Global Variable At Index(CustomPortalEndpoint, Global.EditSelected, Add, Multiply(Vector(0, If-Then-Else(Is Button Held(Event Player, Button(Jump)), 0.1, If-Then-Else(Is Button Held(Event Player, Button(Crouch)), -0.1, 0)), 0), If-Then-Else(Is Button Held(Event Player, Button(Secondary Fire)), 1, 0.3)));
                Modify Global Variable At Index(CustomPortalEndpoint, Global.EditSelected, Add, World Vector Of(Multiply(Throttle Of(Event Player), If-Then-Else(Is Button Held(Event Player, Button(Secondary Fire)), 0.25, 0.08)), Event Player, Rotation));
                Wait(0.016, Ignore Condition);
            End;
        End;
        Stop Camera(Event Player);
...
rule ("Setup and Variables") {
    ...
    actions {
        ...
        Set Global Variable(PortalDest, Empty Array);
        Wait(1, Ignore Condition);
        Set Global Variable(DashExploitToggle, Workshop Setting Toggle(Custom String("Ban (applies to all levels)\n封禁(应用于所有关卡)", Null, Null, Null), Custom String("ban Dash Start - 0关卡Shift", Null, Null, Null), True, 2));
        Set Global Variable(PortalOn, Workshop Setting Toggle(Custom String("map settings \n地图设置", Null, Null, Null), Custom String("enable portals (control maps) - 启用传送门 (占点地图)", Null, Null, Null), True, 10));
        Set Global Variable(CompMode, Workshop Setting Toggle(Custom String("Competitive mode\n竞赛模式", Null, Null, Null), Custom String("Turn on competitive mode - 开启竞赛模式", Null, Null, Null), False, 100));
        If(Global.CompMode);
            "-! comp minutes !- \r\n 5-240"
            Set Global Variable(CompTime, Workshop Setting Integer(Custom String("Competitive mode\n竞赛模式", Null, Null, Null), Custom String("time limit (global) - 时间限制", Null, Null, Null), 120, 1, 240, 101));
            "-! comp attempt count !-"
            Set Global Variable(CompAtmpNum, Workshop Setting Integer(Custom String("Competitive mode\n竞赛模式", Null, Null, Null), Custom String("attempt count - 尝试次数", Null, Null, Null), 5, 0, 500, 102));
            "-! comp restartlimiter !-"
            Set Global Variable(CompRestartLimit, Workshop Setting Toggle(Custom String("Competitive mode\n竞赛模式", Null, Null, Null), Custom String("disable restart during run - 竞赛中禁用重新开始", Null, Null, Null), False, 103));
        Else;
        ...
        Set Global Variable(BanBhop, If-Then-Else(Compare(Count Of(Global.BanBhop), >, 0), Remove From Array(Global.BanBhop, -1), Empty Array));
        Set Global Variable(BanClimb, If-Then-Else(Compare(Count Of(Global.BanClimb), >, 0), Remove From Array(Global.BanClimb, -1), Empty Array));
        Set Global Variable(BanEmote, If-Then-Else(Compare(Count Of(Global.BanEmote), >, 0), Remove From Array(Global.BanEmote, -1), Empty Array));
        Set Global Variable(BanDead, If-Then-Else(Compare(Count Of(Global.BanDead), >, 0), Remove From Array(Global.BanDead, -1), Empty Array));
        Set Global Variable(BanCreate, If-Then-Else(Compare(Count Of(Global.BanCreate), >, 0), Remove From Array(Global.BanCreate, -1), Empty Array));
        Set Global Variable(BanMulti, If-Then-Else(Compare(Count Of(Global.BanMulti), >, 0), Remove From Array(Global.BanMulti, -1), Empty Array));
        "[i for i in BanTriple if i != -1] if len(BanTriple) > 0 else [] # legacy code, now auto sets it to null to save space"
        Set Global Variable(BanTriple, Null);
...
rule ("Player Initialize") {
    ...
    actions {
        ...
        Set Player Variable(Event Player, bouncetouched, -1);
        ...
        Else If(Compare(Hero Of(Event Player), !=, Hero(Genji)));
            Set Player Variable(Event Player, DontPlay, True);
            Abort;
        End;
        Set Player Variable(Event Player, LockEditor, False);
...
rule ("SUB | Checkpoint Fail") {
    ...
    actions {
        ...
        Start Rule(CheckUlt, Restart Rule);
        Start Rule(CheckDash, Restart Rule);
        Call Subroutine(TriggerOnFailSuccesReset);
    }
}
...
rule ("SUB | Start Game") {
    ...
    actions {
        If(And(Global.CompMode, Or(Compare(Global.CompTime, <, 1), (Event Player).CompDone)));
            ...
            Abort;
        End;
        If(And(Global.DashExploitToggle, Compare(Count Of(Global.A), >, 0)));
            Set Ability 1 Enabled(Event Player, False);
...
rule ("Huds: Global/Localplayer") {
    ...
    actions {
        ...
        "global huds"
        Create HUD Text(All Players(All Teams), Null, If-Then-Else(Or(Compare(String("Capture", Null, Null, Null), ==, Custom String("捕捉", Null, Null, Null)), Compare(Custom String("{0}", Color(Rose), Null, Null), ==, Custom String("玫红", Null, Null, Null))), Custom String("房间将在 {0} 分钟后重启 - V{1}", Global.TimeRemaining, Custom String("1_9", Null, Null, Null), Null), Custom String("Server Restarts In {0} Min - V{1}", Global.TimeRemaining, Custom String("1_9", Null, Null, Null), Null)), Null, Right, -161, Null, Color(Red), Null, Visible To and String, Visible Always);
...
rule ("Huds: each player") {
    ...
    actions {
        ...
        Create HUD Text(If-Then-Else((Event Player).LeaderboardToggle, Null, Event Player), If-Then-Else(Or(Compare(String("Capture", Null, Null, Null), ==, Custom String("捕捉", Null, Null, Null)), Compare(Custom String("{0}", Color(Rose), Null, Null), ==, Custom String("玫红", Null, Null, Null))), If-Then-Else(And(Compare((Event Player).PreviewsI, <, Count Of((Event Player).BouncePosition_Cache)), Compare(Count Of((Event Player).PreviewsArray), >, 0)), Custom String(" 弹球弹力: {0} \n 弹球技能: {1} {2} ", Value In Array(Global.EditMode, Value In Array((Event Player).PreviewsArray2, (Event Player).PreviewsI)), If-Then-Else(Value In Array(Global.TQ5, Value In Array((Event Player).PreviewsArray2, (Event Player).PreviewsI)), Ability Icon String(Hero(Genji), Button(Ultimate)), Custom String("", Null, Null, Null)), If-Then-Else(Value In Array(Global.TQ6, Value In Array((Event Player).PreviewsArray2, (Event Player).PreviewsI)), Ability Icon String(Hero(Genji), Button(Ability 1)), Custom String("", Null, Null, Null))), Custom String("", Null, Null, Null)), If-Then-Else(And(Compare((Event Player).PreviewsI, <, Count Of((Event Player).BouncePosition_Cache)), Compare(Count Of((Event Player).PreviewsArray), >, 0)), Custom String(" orb strength: {0} \n orb abilities: {1} {2} ", Value In Array(Global.EditMode, Value In Array((Event Player).PreviewsArray2, (Event Player).PreviewsI)), If-Then-Else(Value In Array(Global.TQ5, Value In Array((Event Player).PreviewsArray2, (Event Player).PreviewsI)), Ability Icon String(Hero(Genji), Button(Ultimate)), Custom String("", Null, Null, Null)), If-Then-Else(Value In Array(Global.TQ6, Value In Array((Event Player).PreviewsArray2, (Event Player).PreviewsI)), Ability Icon String(Hero(Genji), Button(Ability 1)), Custom String("", Null, Null, Null))), Custom String("", Null, Null, Null))), If-Then-Else(Or(Compare(String("Capture", Null, Null, Null), ==, Custom String("捕捉", Null, Null, Null)), Compare(Custom String("{0}", Color(Rose), Null, Null), ==, Custom String("玫红", Null, Null, Null))), If-Then-Else((Event Player).BounceLockMax_Cache, Custom String("黄弹球 {0} / {1}", Count Of((Event Player).LockCollected), (Event Player).BounceLockMax_Cache, Null), Custom String("", Null, Null, Null)), If-Then-Else((Event Player).BounceLockMax_Cache, Custom String("orange orbs {0} / {1}", Count Of((Event Player).LockCollected), (Event Player).BounceLockMax_Cache, Null), Custom String("", Null, Null, Null))), Custom String("{0}{1}{2}", If-Then-Else((Event Player).banstring, (Event Player).banstring, Custom String("", Null, Null, Null)), If-Then-Else((Event Player).banstring, Custom String("\n", Null, Null, Null), Custom String("", Null, Null, Null)), Custom String("{0} {1} / {2}", If-Then-Else(Or(Compare(String("Capture", Null, Null, Null), ==, Custom String("捕捉", Null, Null, Null)), Compare(Custom String("{0}", Color(Rose), Null, Null), ==, Custom String("玫红", Null, Null, Null))), Custom String("检查点", Null, Null, Null), Custom String("Level", Null, Null, Null)), (Event Player).A, Subtract(Count Of(Global.A), 1))), Top, -173, Color(White), Color(Orange), Color(White), Visible To and String, Default Visibility);
...
rule ("Preview orbs/portals | Hold Primary") {
    ...
    actions {
        ...
        "eventPlayer.PreviewsArray = [] # prevent preview empty bug\r\neventPlayer.PreviewsArray2 = []"
        Set Player Variable(Event Player, PreviewsArray, Filtered Array(Global.TQ, Compare(Value In Array(Global.pinballnumber, Index Of Array Value(Global.TQ, Current Array Element)), ==, (Event Player).A)));
        Set Player Variable(Event Player, PreviewsArray2, Mapped Array(Global.pinballnumber, Current Array Index));
        Set Player Variable(Event Player, PreviewsArray2, Filtered Array((Event Player).PreviewsArray2, Compare(Value In Array(Global.pinballnumber, Current Array Element), ==, (Event Player).A)));
        If(Compare(Count Of(Global.CustomPortalStart), >, 0));
            Modify Player Variable(Event Player, PreviewsArray, Append To Array, Filtered Array(Global.CustomPortalStart, Compare(Value In Array(Global.CustomPortalCP, Index Of Array Value(Global.CustomPortalStart, Current Array Element)), ==, (Event Player).A)));
        End;
...
rule ("Restart Run | Crouch + Interact + Deflect") {
    ...
    conditions {
        ...
        Is Button Held(Event Player, Button(Interact)) == True;
        Is Button Held(Event Player, Button(Ability 2)) == True;
        Or(Compare((Event Player).LockEditor, ==, False), Compare(Event Player, !=, Host Player)) == True;
    }
    actions {
...
rule ("Enter Spectate | Hold Interact") {
    ...
    conditions {
        ...
        Or(Compare((Event Player).EditorOn, ==, False), Not(Or(Is Button Held(Event Player, Button(Primary Fire)), Is Button Held(Event Player, Button(Secondary Fire))))) == True;
    }
    actions {
...
rule ("Quick Reset | Reload, Hold Reload to Enable") {
    ...
    actions {
        If((Event Player).quick_restart);
            Abort If(And((Event Player).LockEditor, Compare(Event Player, ==, Host Player)));
...
rule ("Toggle Hud | hold secondary") {
    ...
    conditions {
        ...
        And(And((Event Player).EditorOn, Compare(Event Player, ==, Host Player)), Is Button Held(Event Player, Button(Melee))) == False;
        Global.EditorMoveItem == False;
    }
    actions {
...
rule ("Custom portals | function") {
    ...
    conditions {
        Count Of((Event Player).PortalStart_Cache) > 0;
        Filtered Array((Event Player).PortalStart_Cache, Compare(Distance Between(Current Array Element, Add(Position Of(Event Player), Vector(0, 0.2, 0))), <, 1.3)) != False;
    }
    actions {
        ...
        Teleport(Event Player, Value In Array((Event Player).PortalEnd_cache, Index Of Array Value((Event Player).PortalStart_Cache, First Of(Sorted Array((Event Player).PortalStart_Cache, Distance Between(Event Player, Current Array Element))))));
...
rule ("Comp Mode instruction message - 竞赛模式指引消息 <---- INSERT HERE / 在这输入") {
    ...
    actions {
        ...
        Set Global Variable At Index(instructiontext, 0, Custom String("Change in Comp Mode instruction message hud 1", Null, Null, Null));
        Set Global Variable At Index(instructiontext, 1, Custom String("Change in Comp Mode instruction message hud 2", Null, Null, Null));
        Set Global Variable At Index(instructiontext, 2, Custom String("Change in Comp Mode instruction message hud 3", Null, Null, Null));
        Set Global Variable At Index(instructiontext, 3, Custom String("Change in Comp Mode instruction message hud 4", Null, Null, Null));
    }
}
...
disabled rule ("Title Data - 标题数据 <---- EDIT ME / 在此处编辑") {
    ...
    actions {
        ...
        Wait(1, Ignore Condition);
        "checkpoint number \r\n 每关数量"
        Set Global Variable At Index(TitleData, 0, Array(0, 10, 20, 30, 40, 50));
        "title \r\n 标题文本"
        Set Global Variable At Index(TitleData, 1, Array(Custom String("Bunny", Null, Null, Null), Custom String("Jumper", Null, Null, Null), Custom String("Ninja", Null, Null, Null), Custom String("Pro", Null, Null, Null), Custom String("Expert", Null, Null, Null), Custom String("Master", Null, Null, Null)));
        "color\r\n 颜色"
        Set Global Variable At Index(TitleData, 2, Array(Color(Lime Green), Color(White), Color(Yellow), Color(Orange), Color(Purple), Color(Red)));
    }
}
...
disabled rule ("HUD text for certain Checkpoints - 某些检查点的HUD文本 <---- EDIT ME / 在此处编辑") {
    ...
    actions {
        ...
        Set Global Variable(CpHudText, Array(Custom String("text cp 1", Null, Null, Null), Custom String("text cp 3", Null, Null, Null)));
...
disabled rule ("Inworld text for certain Checkpoints - 在世界文本中为某些检查点 <---- EDIT ME / 在此处编辑") {
    ...
    actions {
        ...
        Set Global Variable(CpIwtText, Array(Custom String("text cp 1", Null, Null, Null), Custom String("text cp 3", Null, Null, Null)));
        ...
        Set Global Variable(CpIwtPos, Array(Vector(0, 0, 0), Vector(0, 0, 0)));
...
disabled rule ("Grup up | text/effect -") {
    event {
        Ongoing - Global;
    }
    actions {
        "replace 777 with checkpoint number\r\n replace vector 0,0,0 with orb position"
        Create In-World Text(Filtered Array(All Players(All Teams), Compare((Current Array Element).A, ==, 777)), Custom String("{0} grup up {0}", Ability Icon String(Hero(Genji), Button(Ultimate)), Null, Null), Vector(0, 0, 0), 1.5, Do Not Clip, Visible To, Color(Orange), Default Visibility);
        "replace 777 with checkpoint number\r\n replace vector 0,0,0 with orb position\r\n 3.5 is the radius"
        Create Effect(Filtered Array(All Players(All Teams), Compare((Current Array Element).A, ==, 777)), Sphere, Color(Orange), Vector(0, 0, 0), 3.5, Visible To);
    }
}

disabled rule ("Grup up | function") {
    event {
        Ongoing - Each Player;
        All;
        All;
    ...
    conditions {
        ...
        (Event Player).C == False;
        "replace vector 0,0,0 with orb position\r\n 3.5 is the radius"
        Distance Between(Event Player, Vector(0, 0, 0)) < 3.5;
    }
    actions {
        Small Message(Event Player, Custom String("   stay in the bubble", Null, Null, Null));
        Wait(1, Abort When False);
        Small Message(Event Player, Custom String("   9", Null, Null, Null));
        Wait(1, Abort When False);
        Small Message(Event Player, Custom String("   8", Null, Null, Null));
        Wait(1, Abort When False);
        Small Message(Event Player, Custom String("   7", Null, Null, Null));
        Wait(1, Abort When False);
        Small Message(Event Player, Custom String("   6", Null, Null, Null));
        Wait(1, Abort When False);
        Small Message(Event Player, Custom String("   5", Null, Null, Null));
        Wait(1, Abort When False);
        Small Message(Event Player, Custom String("   4", Null, Null, Null));
        Wait(1, Abort When False);
        ...
        Wait(1, Abort When False);
        Start Forcing Player Position(Event Player, Add(Value In Array(Global.A, Add((Event Player).A, 1)), Vector(0, 0.1, 0)), True);
        Cancel Primary Action(Event Player);
...
disabled rule ("Custom script on fail, succes, reset, skip") {
    event {
        Subroutine;
...
disabled rule ("Custom Orb Script") {
    event {
        Ongoing - Each Player;
        All;
        All;
    ...
    actions {
        ...
        Set Player Variable(Event Player, BounceTrueId, Mapped Array(Global.pinballnumber, Current Array Index));
        Set Player Variable(Event Player, BounceTrueId, First Of(Filtered Array((Event Player).BounceTrueId, And(Compare(Value In Array(Global.pinballnumber, Current Array Element), ==, (Host Player).A), Compare(Value In Array(Global.TQ, Current Array Element), ==, Value In Array((Event Player).BouncePosition_Cache, (Event Player).bouncetouched))))));
        ...
        If(Array Contains(Array(2), (Event Player).BounceTrueId));
            "example gravity (should be reset to 100 in trigger on fail)"
            Set Gravity(Event Player, 25);
            Small Message(Event Player, Custom String(" you feel light", Null, Null, Null));
            Wait(2, Ignore Condition);
            Set Gravity(Event Player, 100);
        End;
        If(Array Contains(Array(2, 3), (Event Player).BounceTrueId));
            "example canceling primary makes double jump recover"
            Cancel Primary Action(Event Player);
            Small Message(Event Player, Custom String(" double jump recovered", Null, Null, Null));
        End;
        If(Array Contains(Array(4), (Event Player).BounceTrueId));
            "example move speed"
            Set Move Speed(Event Player, 250);
            Small Message(Event Player, Custom String(" zooom", Null, Null, Null));
    }
}
